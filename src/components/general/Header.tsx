          ...
          {isLoading ? <div className="h-11 mt-2 mr-1"><Skeleton className="h-9 w-9 rounded-full" /></div> :
            isAuthenticated ? (
              <div className="flex items-center justify-between space-x-4 p-2">
                <Link href={`/notitications`} className="hidden lg:flex">
                  <Button size='icon' variant='classic'
                    className="bg-[#1b1b1b] bg-opacity-50 h-[2.15rem] w-[2.15rem] rounded-full relative flex justify-center items-center">
                      <NotificationIcon size="18px"/>
                      <span className="absolute top-[5px] right-[8px] rounded-full h-[7px] w-[7px] bg-primary"></span>
                  </Button>
                </Link>
                <ProfileDropdownMenu />
              </div>
              ...
              <div className="hidden sm:flex items-center justify-between space-x-2">
                ...
                <Link href={loginUrl}>
                  <Button className="text-black hover:text-white bg-white rounded-full font-semibold bg-white/95"
                    size="lg">
                    Log in
                  </Button>
                </Link>
